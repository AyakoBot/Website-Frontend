...
const { guildid } = useRouter().currentRoute.value.params;
const { punishmentid } = useRouter().currentRoute.value.params;
...
const updateAppeal = (answer: {
 ...
 const index = answers.value.findIndex((a) => a.uniquetimestamp === answer.uniquetimestamp);
 if (index === -1) answers.value.push(answer);
 else answers.value[index] = answer;

 canAppeal.value = answers.value.every((a) => a.value !== null || a.value !== '');
 answers.value = answers.value.filter((a) => a.value !== null && a.value !== '');
};
</script>
...
<template>
 ...
 >
  <Question v-for="q in props.questions" :key="q.question" :q="q" @update-appeal="updateAppeal" />
  <input

...
const handleHovered = (review: ReviewType | null) => {
 ...
 if (review) {
  ...
  interval = setInterval(() => {
   if (timesContentChanged === hoveredReview.value?.content.length && interval) {
...
<template>
 <div>
  <Suspense>
   <template #default>
    <ReviewDisplay @review-hovered="(review: ReviewType | null) => handleHovered(review)" />
   </template>
   <template #fallback>
    <ReviewLoading />
   </template>
  ...
  >
   <div class="absolute z-10 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 text-md w-10/12">
    <div class="flex flex-col m-auto justify-center items-center reviewContent">
  ...
  <div class="relative">
   <div class="h-[30rem] w-11/12 absolute left-1/2 -translate-x-1/2 -top-[30rem]">
    <div class="shadow-main rounded-2xl h-full w-full flex flex-col justify-around items-center">
